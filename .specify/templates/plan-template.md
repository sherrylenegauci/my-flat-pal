...
| 2 | UI designed at 375px first; 44x44px touch targets; semantic HTML; operable without touch (VoiceOver on a real device is the check that counts; automated keyboard traversal is supporting evidence only) with a visible focus indicator; **text alternatives on meaningful images and icons**; WCAG 2.1 AA contrast; **verified per view before the feature is complete, not deferred to polish** | II. Accessibility & Mobile-First | [PASS/FAIL/PLANNED] |
...
| 3b | Feature declares where it lives in the app's structure; shell changes treated as cross-cutting, not feature-owned | IV. One App, Several Features | [status] |
| 3c | No colour, font size or spacing defined outside `src/ui/tokens.css`; every contrast ratio computed and recorded, never estimated; meaning never carried by colour alone | V. One Visual Identity | [status] |
