...
- [ ] T078 **Run the full manual device checklist in plan.md § Running and checking it on a real iPhone and a real Android phone** — SC-006 and Constitution gate 2b. Not automatable
  ...
  - **Extended 2026-08-22 by T115, T120 and T121, and none of the three is more of the same.** (1) **The period dropdown re-labels itself under the user.** Its options now read "year" at a count of 1 and "years" at any other, so the text of the option a person is sitting on changes while they edit the number beside it. In a closed `select` that is settled — React rewrites the text, the value does not move, and focus stays in the count box, all asserted in `tests/ui/form-interval-plural.test.tsx`. On iOS the control is a **wheel picker**, and whether VoiceOver re-announces an option whose text changed under it is a device question nothing here can answer. Check that the picker still says the right period after the count changes, and that it is announced rather than silently different. (2) **A whole job card is now one hit area.** The name's pointer target is stretched over the card and "Mark done" is raised above it; the browser tier establishes which control receives a *pointer* event, in both engines, including that Mark done still records a completion in storage. What it cannot establish is what a **finger** does: `touch-action`, a tap highlight that now flashes across a whole card, whether a drag down the list cancels the tap, and what a long press does. A card-sized target also makes an accidental open cheaper to trigger than a name-sized one did — check it one-handed on a moving list. And the name no longer carries an underline, so check that a VoiceOver user still meets it as a button and that a sighted user can tell the card is tappable at all. (3) **The "Last done" field is now sized by the engine** rather than to the form, via `fit-content`. iOS Safari is not the WebKit the browser tier runs and it draws its own date control, so check on the device that "dd/mm/yyyy" and the calendar icon are not clipped, and that a 118px-wide target is still comfortable to hit.
...
- [X] T110 **The whole palette is in `tokens.css`, and the two literals that stay say why** (`feat/004-tidy-up`, 2026-08-22). Constitution V puts colour in one file; `src/ui/focus.css` had been declaring its own `--focus-ring` and `--focus-ring-inner` and `src/ui/app.css` stating four colours in declarations, all of it predating the amendment.

  **Six literals, six tokens.** `--focus-ring` and `--focus-ring-inner` moved unchanged. Three are new names rather than moves: `--text-on-accent` (the word on a filled button, which was `color: #fff` twice), `--shadow-dialog`, and `--scrim`. `--text-on-accent` is deliberately *not* `--surface` even though the two whites are byte-identical today — a card's fill and a button label's colour answer different questions, and the first time they disagree, which dark mode would do, a shared token would have to be split with every call site already assuming it meant both.

  **The focus ring's geometry stayed in `focus.css`.** `--focus-ring-width` and `--focus-ring-offset` are that component's dimensions, not the spacing scale, and moving them into the palette would say they were tunable when they are load-bearing for visibility. The seven measured contrast pairs stayed there too, beside the rule that uses them, which is where someone changing the ring will be standing.

  **Both predicted exceptions were real, and both stand.** A hex custom property cannot be given an alpha — `rgba(var(--text), 0.6)` is invalid, because `var()` substitutes the whole `#0e141b` rather than three numbers — so the scrim restates `--text`. `color-mix()` could do it and was not used: Principle I, and it would trade a restatement anyone can read for a computation fewer people can. **But "cannot be derived" is not "cannot be moved", and the task line conflated the two.** The restatement was six hundred lines into `app.css` under a comment asking whoever moved the palette to remember it; it is now in `tokens.css` under `--text`'s own nose, which is the whole point of Principle V. The `select` chevron genuinely cannot move: a data URI parses as an opaque string, so `var()` inside it is text. It is the one allowlisted literal and it has a drift check of its own.

  **The check is `tests/assets/palette-single-source.test.ts`**, and it is a guard rather than a one-off cleanup: it discovers `src/ui/*.css` with `readdirSync`, so a new stylesheet arrives already covered, and it strips comments first, because these files quote hexes and recorded ratios and a guard that flagged documentation would be turned off. Red before the fix, naming all six literals by file and line. It also asserts `app.css` holds exactly one data URI — the allowlist exempts *a line carrying one*, so a second would inherit the chevron's exemption.
- [X] T119 **The manifest's colours are checked against the tokens, not remembered** (`feat/004-tidy-up`, 2026-08-22). `theme_color` in `vite.config.ts`, `background_color` beside it, and the `theme-color` meta in `index.html` held the app's *original* palette through two complete design passes, so an installed app opened with a status bar belonging to neither palette.

  Both were done, not either: there is now a note at `--surface` and `--surface-sunken` in `tokens.css` naming the two files they also ship in, **and** three assertions in `tests/assets/palette-single-source.test.ts` comparing them. The note alone was what failed last time.

  The checks read the source files rather than `dist/`. A build would prove the same thing 30-60 seconds slower and prove it about the wrong artifact — the recorded failure is a person moving the tokens and forgetting these two files, which is visible in the source the moment it happens. Both were green on arrival, so they were sabotage-proven instead: a one-unit change to `--surface` reddens the `vite.config.ts` and `index.html` assertions and leaves `background_color` alone; a one-unit change to `--surface-sunken` does the reverse.

  **What no tier can see, and it is the part that matters.** `theme_color` is painted by the OS into a status bar and `background_color` into a launch splash, both before the app has run a line of JavaScript. No headless environment shows you either. This stops the *source of truth* drifting; whether the phone honours it is still a real-device check on the manual checklist.

  One incidental finding, recorded because it nearly made the check vacuous: stripping `/* */` comments from `vite.config.ts` the way the CSS guard does blanks forty lines of it, because `'icons/*.png'` opens a block comment that does not exist and `'**/*.test.ts'` closes it. The manifest reader strips only whole-line `//` comments for that reason, and each CSS guard asserts its file still contains a `{` after stripping — a guard with nothing left to scan reads exactly like a guard that passed.
...
- [X] T114 **The repository refuses an interval the domain cannot compute with** (`feat/004-tidy-up`, 2026-08-22). The task below offered two answers and required exactly one; the answer taken is **the repository rejects the document**, because the other one — the spec saying what a nonsense interval reads as — means inventing a meaning for an interval of zero in order to print it.

  **The task understated the damage, and that changed how the fix was written.** It reads as a display bug: `"1" === 1` is false, so `{ count: "1", unit: "year" }` renders "Every 1 years". But `addInterval` in `src/domain/interval.ts` *throws* on a count that is not a whole number of at least 1, and its `switch` on `unit` has no `default`, so an unrecognised unit returns `undefined` and `formatDisplayDate` then throws `Not a calendar date: undefined`. A bad interval does not misprint a line — it fails the app while working out what is due, on load, on the only document there is, with no way back in. **So `unit` is validated as well as `count`**, which the task did not ask for: validating the count and leaving the unit would have fixed the cosmetic half of a crash.

  The contract is the existing one for a document of the wrong shape (`load()` returns `corrupt`, the original is parked under a recovery key byte for byte, the app gets a usable empty schedule). `isAComputableInterval` uses `Number.isInteger`, the same test `addInterval` uses, deliberately: two notions of "a valid count" in two layers is how one of them quietly stops protecting the other.

  **The cost, recorded rather than glossed.** One uncomputable interval now rejects the *whole* document, so a single hand-edited job takes the entire schedule to a `.recovered.*` key — and that key still has no path back to the user, which is already logged in the Notes above as an open gap. That is the same trade the repository already made for a bad `name` or a bad `id`; it is not a new policy, but it is a bigger blast radius than the task's framing suggests, and it is the reason 25 of the 39 tests in `tests/storage/recovery.test.ts` are about this one check. Five positive cases sit beside them so the rejection cannot be satisfied by refusing everything.

  **What this task said before it was worked**, kept because its reasoning is what made the answer a choice rather than an oversight: *[P] `interval.count` is validated nowhere, and one display rule now leans on it.** `src/storage/repository.ts` checks only `typeof it['interval'] === 'object'` — nothing establishes that `count` is a number, an integer, or positive. The add form refuses those (`ItemFormView`), so today the only way in is a hand-edited document or a future writer, but `formatInterval`'s `count === 1` is indistinguishable from `count <= 1` given any fixture set a test would reasonably use, and `"1" === 1` is false, so `count: "1"` would render "Every 1 years". Found by sabotage during T113 and **left alone on purpose**: what the app should display for a count of zero is undefined by the spec, so a test asserting an answer would be writing the spec rather than checking it. Either the repository rejects the document, or the spec says what a nonsense interval reads as. Not both, and not neither.*
- [X] T115 **The form pluralises the period off the count** (`feat/004-tidy-up`, 2026-08-22). Sherrylene chose option 2 below, so the row now reads "Every 1 year" and "Every 3 months" and agrees with the detail view beside it.

  `UNITS` stopped being a table of labels and became the four `IntervalUnit` values, with a one-line `periodLabel(unit, count)` beside it that adds an s unless the count is 1 — the same suffix decision, for the same reason, that `formatInterval` records. **The count decides, never the unit**: keying off the unit reads correctly on the annual job that prompts the change and turns a quarterly filter into a monthly one, which is why `tests/ui/form-interval-plural.test.tsx` walks all four units at counts 1, 2, 3 and 10 rather than the yearly case anyone would think of. The count is form state and therefore *text*, so the comparison is `Number(count) === 1`: "01" is singular, "" and anything half-typed is plural, which is the right answer for zero in English and the safe one mid-edit.

  **Labels moved; values did not, and that is asserted separately.** The stored unit is still `'day' | 'week' | 'month' | 'year'`. An implementation that renamed both would read correctly on screen and corrupt every document the app has written, and there is no export path to recover from — so the option values are checked in their own tests rather than left implied by the labels. Saving runs under `<StrictMode>` and reads the stored document back, on the add path and on the edit path.

  **The objection recorded against this option was tested and does not hold for a closed `select`**: React rewrites the option text in place, the value does not move, and focus stays in the count box — asserted. **What is still unverified is the iOS wheel picker**, where whether VoiceOver re-announces an option whose text changes under the user is a device question this repository cannot answer. Routed to T078, which now names it, rather than claimed here.

  **The task as it stood before it was worked**, kept because the four options are what made this a decision rather than an oversight:

  *[P] **The edit form is now the only place in the app that says "every 1 years".*** `ItemFormView` renders "How often — every | 1 | years" as a label, a number input and a period dropdown, so opening the annual boiler job for editing shows the grammar T113 removed from the view beside it. It is a control rather than a statement of the schedule, which is why it is not the same defect — a dropdown cannot sensibly re-label itself as the number changes without moving focus under the user. Recorded because it is the last survivor, and because the fix is a copy decision rather than a technical one
  ...
  **Picked up on `feat/004-tidy-up` 2026-08-22 and put back down, deliberately.** It was handed over as work alongside T110, T114 and T119. It is not work yet: this line says the decision is Sherrylene's and unasked, and writing the copy would have made that decision silently while the file went on claiming it was open. Nothing was changed. What the investigation adds is the shape of the question, so that answering it is one sentence rather than a fresh investigation.

  The plurals are a static table — `UNITS` at the top of `src/ui/views/ItemFormView.tsx`, `days`/`weeks`/`months`/`years`. Four candidate answers, and they are not equivalent:

  1. **Leave it.** It is a control, not a statement, and "Every | 1 | years" is how most forms of this shape read. Costs nothing and the inconsistency with the detail view stays.
  2. **Pluralise the option labels off the current count**, so the row reads "Every 1 year" and "Every 3 months". This is what `formatInterval` already does and it would make the two agree on the rule. It reads "Every 1 year", never "Every year" — the number input cannot disappear. The objection in the line above, that a dropdown cannot re-label itself without moving focus, looks unfounded for a *closed* `select`: React rewrites the option text and neither value nor focus moves. **It is not verified on iOS**, where the control is a wheel picker and whether VoiceOver re-announces the changed option is a device question, which is exactly the class of claim this project has been wrong about before.
  3. **Singular labels throughout** — "Every | 3 | month". Reads correctly at 1 and wrongly everywhere else, which is the mistake `formatInterval`'s comment already warns against.
  4. **Add a read-back sentence** under the fieldset, from `formatInterval`, saying "This job comes round every year." Makes the form state the schedule in the app's own words at every count. It also adds a line to a form on a 375px screen, and it is an addition rather than a copy change, so it wants a look before it is built.

  Only Sherrylene picks. Whichever is chosen, the seven test files that drive this field are already re-anchored from T116 and the change is small; option 4 is the only one that needs new tests beyond the existing item-form ones
...
- [X] T120 **The whole job card opens the job, and the name stops being underlined** (`feat/004-tidy-up`, 2026-08-22). Reported by Sherrylene from using the app on her iPhone 14, in her words: "we don't need to highlight the text to make it tappable area, we can just make the whole card tappable." The underline had looked wrong since the design pass. It was the only thing saying the name did something — `app.css` said so in as many words — and it was the wrong answer to that: a name long enough to wrap read as two links, and three names down the page read as link soup. The affordance is now the card, which is what a thumb was reaching for anyway.

  **The card cannot become a button, and that constraint is what shapes the fix.** The card also holds "Mark done", and a button inside a button is invalid HTML that assistive technology reads unpredictably. So the name stays the one control — same accessible name, same place in the focus order, same focus ring on the text, since an outline is drawn on an element's own box and a positioned pseudo-element does not move it — and only its *pointer* target grows: `.row__open::after` with `inset: 0` inside a `position: relative` card. Nothing about the accessibility tree changes, which is why no new behaviour-tier test was needed for the name.

  **`z-index: 1` on `.row__done` is the load-bearing line in the change, and the failure it prevents is silent.** With the overlay on top, tapping Mark done opens the job: nothing errors, nothing looks wrong, and the completion the user believes they recorded does not exist. That is strictly worse than the underline it replaces. The **button** is raised rather than `.row__actions`, and the difference is visible to a user — the actions row spans the card, so raising the container would leave a dead strip beside the button that opened nothing. Both points are tapped by `e2e/row-tap.spec.ts`.

  **Everything here is hit-testing, which is precisely what the tier below cannot see.** jsdom lays nothing out and does no hit-testing: a click dispatched at an element is delivered to that element whatever is drawn over it, so a behaviour test would have reported that Mark done works while a real thumb opened the job. `e2e/row-tap.spec.ts` clicks **coordinates** rather than locators, because Playwright's own actionability check refuses to click a covered element and reports it as a timeout naming the coverer — a red run about Playwright rather than about the app, stopping before any assertion about what the app did. And its third test asserts the **stored document**, not only the screen: a completion that never reached `localStorage` is one the user loses on the next launch, and this app has no export.

  **Sabotage-proven, both ways, in both engines.** Removing `z-index: 1` reddens "tapping Mark done records the completion and stays on the list" with "no undo offer naming the job appeared, so nothing was recorded"; removing `.row__open::after` reddens both card-tap tests. `--underline-offset` was deleted from `tokens.css` in the same change: it described how far a line cleared the descenders, and there is no line.

  **Not tested, deliberately: that the underline is absent.** Asserting "no text-decoration" pins the absence of a decoration a later design pass might legitimately restore in some other form, and it is a stylistic fact rather than a behaviour — the same reasoning T113 recorded for the history entries no longer being cards.

  **The focus ring and the hit area now disagree, and nothing says whether that is right.** The ring is drawn on the name button's own box, so it hugs the words while the tap target is the whole card. `e2e/focus-visibility.spec.ts` goes on measuring the ring correctly and has nothing to say about the mismatch. A ring on the words with a card-sized target is a legitimate design — the words are what the control *is* — or a confusing one, and it is a judgement on a device rather than a check. Raised by the test agent; recorded here rather than guessed at.

  **What no tier here reaches**, and it is the half a finger does rather than a pointer: `touch-action`, the tap highlight now covering a whole card, whether an accidental drag down the list cancels the tap, and what a long press does. Card-sized hit areas make an accidental open cheaper to trigger than a name-sized one did. On the manual checklist (T078).

- [X] T121 **The "Last done" field is sized to a date rather than to the form** (`feat/004-tidy-up`, 2026-08-22). Reported by Sherrylene from her iPhone 14 and then measured: on a 390px viewport in WebKit the field was **355x44** — the full width of the form and the same width as the Name box, which takes a whole job name — for a control that shows "dd/mm/yyyy" and a calendar icon. It inherited `width: 100%` from the shared `input[type='text'], input[type='number'], input[type='date'], select` rule. It is now **118.1x44** in the same measurement.

  **`fit-content`, not a number.** The intrinsic width of `input[type=date]` is the engine's own and they disagree — 118.1px in WebKit against 163.8px in Chromium. A pinned width would be a test of one engine's form-control metrics, and asking each engine for its own answer is also what stops the field clipping on iOS Safari, which is not the WebKit the browser tier runs. `width` rather than `align-self`, so it does not depend on `.form__field` being a flex container: a stretched flex item ignores `width: auto`, but a definite width turns the stretch off.

  **The height is untouched at 44px**, which is not a detail: Principle II's touch-target floor is a MUST, and a narrower field must not become a shorter one. The `min-height: var(--touch-target)` in the shared rule still applies, and the existing 44x44 sweep in `e2e/layout.spec.ts` covers it for every state.
