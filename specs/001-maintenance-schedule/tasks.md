...
- [ ] T117 **The app bundles its own typeface, and the type scale was re-judged around it** (`design/type-a-source-sans`, `design/type-b-plex`, `design/type-c-newsreader`, 2026-08-20). Reported by Sherrylene from using the app: the fonts look **generic**, and the **weights and sizes** are wrong. Two complaints, and they need different answers.

  **Open: which candidate.** Three are built, each on its own branch, each complete — vendored woff2, self-hosted, precached, OFL 1.1, variable weight axis, tests, and the plan's dependency budget corrected with that candidate's real shipped size. `design/refresh` is untouched. Nothing is chosen; screenshots of the schedule list and the job detail at 375px are the deliverable, at `~/Desktop/my-flat-pal-typefaces/`. **A — Source Sans 3** alone, 28,740 bytes. **B — IBM Plex Sans** alone, 45,712 bytes, more character and 10% wider. **C — Newsreader over Source Sans 3**, 86,824 bytes, serif headings. When one is chosen the other two branches are deleted and this task records which and why.
  ...
  **What the tests do not establish, and it is not small.** How any of this looks on a real iPhone at 3×. Whether the swap from fallback to bundled face on a first load is a visible reflow worth avoiding. And on the two single-face branches, the `--font-heading` route is checked by nothing: setting that token to something invalid makes the property invalid at computed-value time, so it inherits `--font` — the same face — and every heading is still right, for the wrong reason. It becomes a real check only on branch C, where the token names a different family. Recorded in `e2e/typeface.spec.ts` beside the sample it applies to

- [ ] T118 **The app has a mark, and the icons stopped being two palettes stale** (`design/mark`, `design/mark-b-window`, `design/mark-c-door`, 2026-08-20). Reported by Sherrylene from looking at the app: the logo is **very plain**. It was worse than plain, and the second half of this was not reported by anyone because nobody could see it.
  ...
  **One gap left open on purpose.** Nothing compares the header's rendered SVG against the rasterised PNG. They share `src/ui/mark.ts`, which is what makes them the same drawing, but a scale or transform bug affecting only the component or only the generator would be caught in one tier and invisible in the other. Recorded rather than papered over with a check that could not see it.
