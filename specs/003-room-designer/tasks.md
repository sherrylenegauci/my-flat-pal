...
- [X] T003 [P] Define `Room`, `PlacedObject` and `Piece` in `src/domain/rooms/types.ts` per plan.md § Data model. **Millimetres as integers throughout** — floating-point centimetres make two objects that visibly touch differ by 0.0000001 and the collision check flickers
