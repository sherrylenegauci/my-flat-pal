...
- [ ] T022 **Drive the shell with VoiceOver on a real iPhone.** Constitution v1.4.0 makes this the check that discharges the accessibility gate. Specifically: whether each tab is announced by name, whether the current one is announced as current, whether switching areas lands somewhere that makes sense, and **whether a thumb can comfortably reach a 44px target sitting above the home indicator**. **Not automatable**
  - **Partly answered already, and it found a defect.** Looking at the bar on an iPhone 14 is what turned up T024 — the bar did not reach the bottom of the screen. The rest of this task still needs the device, and still needs rooms to exist before there is a bar on screen to point VoiceOver at.
- [X] T024 **Make the bar reach the bottom edge of the screen on a notched phone.** Found on a real iPhone 14: the bar floated with a strip of the list showing beneath it. `.app` reserves the bottom safe area, so a bar laid out inside it stops short of the edge — and `bottom: env(safe-area-inset-bottom)` put it there deliberately, on the reasoning that a bar must not sit over the home indicator
  - Done in `app.css`. The reasoning was half right: what must not sit over the indicator is the *tappable* part, while the background is expected to run to the edge — which is what every native iOS bar does. The bar now cancels the app's inset for itself with a negative margin and pays it back as padding, so the background extends down and the 44px targets stay above the indicator. Self-contained, so the no-bar case is untouched. **Reproduced before fixing** by simulating a 34px inset in a real browser, where the strip of list below the bar is plainly visible. It also sharpens question 1 below: the old version did not merely cover content, it left content visible *underneath* the bar, which reads as broken rather than as a judgement call
- [X] T025 **Give each tab an icon.** A label-only bar is legible but slow to find with a thumb, and the two areas are told apart faster by silhouette than by reading
  - Done as `src/ui/icons.ts` (geometry) and a `TabIcon` in `TabBar.tsx`, following the split `mark.ts` established — geometry in a plain module, colour in `app.css`, nothing painted here. **The bar did not grow**: an 18px icon over a 15px label comes to 41px inside the 44px minimum, so it is still 45px and T019's measurements are unchanged, which matters because that guard already showed the fourth job one pixel under the bar. Guarded by `tests/ui/tab-icon.test.tsx`, sabotage-proved four ways — removing `aria-hidden`, adding a `<title>`, drawing no icon, and putting a colour literal in the geometry each turn it red and nothing else
  - **The spanner is filled and the sofa is an outline, which is deliberate.** Three outline spanners were drawn and all three read as a curl on a stick at 18px, because a thin ring on a line is a magnifying glass. A filled sofa has the opposite problem — it becomes a lozenge on legs. The sofa's stroke went from 8 to 10 so the pair reads at one weight. Every one of those judgements was made at 18px in a browser, which is the only size that can make them
